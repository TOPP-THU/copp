classdef Result
    %RESULT COPP3-SOCP expert result with Clarabel diagnostics.
    %
    % PROFILE is available only when HAS_PROFILE is true. PROFILE.a and
    % PROFILE.b are s_len-by-1 double columns. Raw Clarabel vectors X, Z, and S
    % and objective_terms are stored as double column vectors.

    properties (SetAccess = private)
        %HAS_PROFILE Whether PROFILE contains an accepted Profile3rd.
        has_profile

        %PROFILE Accepted third-order profile, or [] when unavailable.
        profile
    ...
    methods
        function obj = Result(has_profile, profile, x, z, s, solver_status, ...
                obj_val, obj_val_dual, solve_time, iterations, r_prim, r_dual, ...
                linsolver, objective_value, objective_terms)
            %RESULT Construct a MATLAB-owned COPP3 expert result.
            %
            % Raw vector inputs may be row or column vectors. Stored vector
            % properties are normalized to double columns.
            arguments
                has_profile (1,1) logical
                profile
