"""Use TOPP3-LP to convert a JAX-defined path into a jerk-limited time-optimal trajectory.

The example builds a TOPP2-RA seed, solves a third-order ``(a,b)`` profile with
the Clarabel LP backend, and performs one refinement around the first solution.
"""
...
def main() -> None:
    ...
    # 4) Solve TOPP3-LP twice, refreshing the a(s) linearization once.
    options = copp.solver.topp3_lp.Options(allow_almost_solved=True)
    ...
    print("TOPP3-LP done. (The first iteration)")
    print(f"dim = {dim}, N = {n}")
    ...
    print("---------")
    print("TOPP3-LP done. (The second iteration)")
